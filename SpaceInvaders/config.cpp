...
Config::Config() {
    ...
    QFile c_file(":/Configuration/stage.txt");
...
void Config::setSwarmListLevel()
{
    levelNumberList.append(swarmList.size());
    for(int i = 0; i < levelNumberList.size() - 1; i++){
        int start = levelNumberList.at(i);
        int stop = levelNumberList.at(i+1);
        std::stringstream ss;
        QList<SwarmInfo> information = {};
        for(int k = start; k < stop; k++){
            information.append(swarmList.at(k));
        }
        swarmListLevel.append(information);
    }
...
void Config::processInstructions(QStringList& instructs, QStringList& copy) {
    ...
    for (int i = 0; i < copy.size(); i++) {
        ...
        if (copy[i] == "Left" || copy[i] == "Right" || copy[i] == "Shoot" || copy[i] == "Medium" || copy[i] == "Large") {
...
void Config::processSwarm(QTextStream& in) {
    ...
    while (!in.atEnd()) {
        ...
        if (l.isEmpty()) {
            ...
            levelNumberList.append(levelnumber);
        } else if (l.startsWith("[SWARM]")) {
            levelnumber++;
            //recursion
            processSwarm(in);
...
void Config::saveSwarm(
    ...
    if (positions.size() > 0) {
        swarmList.insert(0,SwarmInfo(type, positions, move, shoot));
    }
}
...
QList<SwarmInfo> Config::getSwarmList(int level) {
    ...
    if(level<this->swarmListLevel.size())
    {
    return this->swarmListLevel.at(level);
    }
    else exit(0);
}
